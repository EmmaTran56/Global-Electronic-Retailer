...
To support these goals, the analysis answers eight core business questions
(implemented as queries in `answer.sql` and illustrated in `assets/`).

1. **Monthly profit trend**: How does profit evolve by year and
   month, and which periods show strong or weak performance that the business
   should plan for?
2. **Cumulative profit and growth**: How does cumulative profit
   evolve over time, and where do we see sustained vs. slowing growth?
3. **Store productivity**: For each store, what is the profit per
   square meter in local currency, and which stores are the most and least
   efficient?
4. **Dynamic order pivot by country and year**: How many
   orders are placed in each country by year, using a dynamic pivot that
   automatically adapts to new years without changing the SQL?
5. **Customer structure**: What does the active customer base
   look like by age group, gender, and location, and which segments are most
   important by size?
6. **Customer order summary on a target date**: For a given
   order date, what are the key details about each customer’s orders (e.g.,
   number of orders, total quantity, total amount) to support daily or
   event‑based analysis?
7. **Product subcategory affinity**: Which product subcategories
   are most frequently purchased together in the same order, and how can this
   inform cross‑sell or bundling strategies?
8. **Top products by country and category**: Within each product
   category and country, which products are the top sellers by quantity (top 2),
   and how can that guide assortment and inventory decisions?
...
At a high level, the relationships between the core tables are:

- **Customers → Sales**: One customer can place many sales orders (1‑to‑many).  
- **Products → Sales**: One product can appear in many sales rows over time (1‑to‑many).  
- **Stores → Sales**: One store records many sales transactions (1‑to‑many).  
- **Exchange_Rates → Sales**: One exchange‑rate entry (for a given date/currency) can be used by many sales rows that occur on that date in that currency (1‑to‑many).

The `Sales` table acts as a **fact table** that links these dimensions together; conceptually, this implements many‑to‑many relationships between **Customers**, **Products**, and **Stores** through the `Sales` fact (for example, a customer can buy many products, and a product can be bought by many customers, but each individual line is a single row in `Sales`).
...
### Question 2 – Cumulative profit and months with ≥ 10% growth

![Q3 result](assets/q3.png)

### Question 3 – Store profit per square meter (local currency)

![Q6 result](assets/q6.png)

### Question 4 – Dynamic pivot of total orders by country and year

![Q8 result](assets/q8.png)

### Question 5 – Customer counts by age group, gender, and location


![Q2 result](assets/q2.png)

### Question 6 – Customer order summary for a target order date


![Q7 result](assets/q7.png)

### Question 7 – Most frequent subcategory pairs in the same order

![Q4 result](assets/q4.png)

### Question 8 – Top 2 products by quantity per category and country
...
- **Orders/profit by country and year**  
...
- **Customer order summary by date**  
...
- **Top products by category and country**  
